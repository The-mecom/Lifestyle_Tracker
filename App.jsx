...
const STORAGE_KEYS = {
  finances: "lt_finances",
  health: "lt_health",
  sleep: "lt_sleep",
  reading: "lt_reading",
};

const initialFinances = { savings: "", investments: "", expenses: [], debts: [] };
const initialHealth = { entries: [] };
const initialSleep = { entries: [] };
const initialReading = { books: [] };
...
export default function LifestyleTracker() {
  ...
  const [finances, setFinances] = useState(() => loadData(STORAGE_KEYS.finances, initialFinances));
  const [health, setHealth] = useState(() => loadData(STORAGE_KEYS.health, initialHealth));
  const [sleep, setSleep] = useState(() => loadData(STORAGE_KEYS.sleep, initialSleep));
  const [reading, setReading] = useState(() => loadData(STORAGE_KEYS.reading, initialReading));

  useEffect(() => { saveData(STORAGE_KEYS.finances, finances); }, [finances]);
  useEffect(() => { saveData(STORAGE_KEYS.health, health); }, [health]);
  useEffect(() => { saveData(STORAGE_KEYS.sleep, sleep); }, [sleep]);
  useEffect(() => { saveData(STORAGE_KEYS.reading, reading); }, [reading]);
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input, select, textarea {
          background: #16161a; border: 1px solid #252529; color: #e8e4dc;
        ...
        .btn-primary {
          background: #c9a96e; color: #0a0a0c; border: none;
        ...
        .btn-ghost {
          background: transparent; color: #555; border: 1px solid #252529;
        ...
        .entry-row {
          ...
          padding: 11px 0; border-bottom: 1px solid #16161a;
        }
        .entry-row:last-child { border-bottom: none; }
        .tag {
          background: #1c1c22; border-radius: 20px;
          padding: 3px 10px; font-size: 11px; color: #666; white-space: nowrap;
        }
        .tab-btn {
      ...
      <div className="app" style={{ maxWidth: 780, margin: "0 auto", padding: "0 16px 60px" }}>
        <div style={{ padding: "28px 0 20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
            <div>
              <p className="label" style={{ marginBottom: 6 }}>Your personal</p>
            ...
            <p style={{ color: "#333", fontSize: 12, textAlign: "right" }}>
      ...
      <div className="card">
        ...
        {(() => {
          const items = [
            ...
            ...health.entries.slice(0, 1).map(e => ({ date: e.date, text: `Health logged${e.weight ? ` · ${e.weight}kg` : ""}`, value: "🫀", color: "#f472b6" })),
